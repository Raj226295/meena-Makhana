...
function BrandLogo({kind,compact=false}){
 if(kind==='visa')return <span className={`brand-logo visa-logo ${compact?'compact':''}`} aria-label="Visa"><b>VISA</b><i/></span>
 if(kind==='paytm')return <span className={`brand-logo paytm-logo ${compact?'compact':''}`} aria-label="Paytm"><b>pay</b><strong>tm</strong></span>
 return <span className={`brand-logo upi-logo ${compact?'compact':''}`} aria-label="UPI"><b>UPI</b><i><span/><span/></i></span>
}
function MethodMark({kind}){return <span className={`payment-method-mark ${kind}`}><BrandLogo kind={kind}/></span>}
function ChoiceGlyph({id,icon}){return <span className={`choice-glyph ${id}`}><i>{id==='upi'?<BrandLogo kind="upi" compact/>:<Icon name={icon}/>}</i><em>{id==='upi'?'Instant':id==='card'?'Secure':id==='bank'?'Direct':id==='wallet'?'Quick':'Flexible'}</em></span>}
...
export default function PaymentMethodsPage(){
 ...
 const [methods,setMethods]=useState(()=>{const saved=read('meena-payment-methods',starterMethods);return Array.isArray(saved)?saved:starterMethods})
 ...
 return <><StoreNavbar wishlistCount={wishlist.length}/><main className="payments-page"><div className="payments-shell"><AccountSidebar active="Payment Methods" wishlistCount={wishlist.length}/><section className="payments-content">
  ...
  <article className="payments-panel add-payment-panel"><header><h2>Add New Payment Method</h2><p>Choose a payment method to add to your account.</p></header><div className="payment-choice-grid">{choices.map(([id,icon,title,text])=><button key={id} className={selected===id?'selected':''} onClick={()=>setSelected(id)} aria-pressed={selected===id}><ChoiceGlyph id={id} icon={icon}/>{selected===id&&<span className="choice-check"><Icon name="check"/></span>}<strong>{title}</strong><small>{text}</small></button>)}</div><div className="payment-trust"><i><Icon name="shield"/></i><div><b>Safe. Secure. Trusted.</b><p>Industry-standard encryption and trusted payment partners keep your information safe.</p></div><div className="payment-partners" aria-label="Supported payment standards"><span className="pci-mark"><Icon name="lock"/><b>PCI</b><small>DSS</small></span><span className="verified-mark"><small>Verified by</small><b>VISA</b></span><span className="mastercard-mark"><i/><i/><small>mastercard</small></span><span className="rupay-mark"><b>RuPay</b><i/></span><BrandLogo kind="upi" compact/></div></div><div className="payment-continue"><span><Icon name="lock"/>No payment details are stored before secure verification.</span><button onClick={continueSetup}>Continue Securely <Icon name="arrow"/></button></div></article>
