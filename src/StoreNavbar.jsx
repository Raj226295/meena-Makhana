...
function navigateTo(url){history.pushState({},'',url);window.dispatchEvent(new PopStateEvent('popstate'));window.scrollTo({top:0,behavior:'smooth'})}
 ...
 return <div className="store-header-space">
  <div className="store-header-shell">
   ...
   <div className="store-actions">
    <button type="button" onClick={onProfile||(()=>navigateTo('/login'))} aria-label="Account"><ActionIcon name="profile"/><span>Account</span></button>
    <button type="button" onClick={onWishlist} aria-label={`Wishlist ${wishlistCount}`}><span className="action-art"><ActionIcon name="wishlist"/><i>{wishlistCount}</i></span><span>Wishlist</span></button>
    <button type="button" onClick={onCart} aria-label={`Cart ${cartCount}`}><span className="action-art"><ActionIcon name="cart"/><i>{cartCount}</i></span><span>Cart</span></button>
   </div>
   </header>
  </div>
 </div>
