...
function App(){
 ...
 const openPanel=(name,event)=>{lastTrigger.current=event?.currentTarget;setPanel(name)}
 ...
 useEffect(()=>{
  ...
  const update=()=>{
   ...
   frame=requestAnimationFrame(()=>{
    ...
    const current=ids.filter(id=>document.getElementById(id)?.getBoundingClientRect().top<=120).pop()
 ...
 return <>
  ...
  <main>
   <section className="reference-hero" id="home" aria-label="Premium Makhana from Bihar"><img src="/hero-premium-banner.png" width="1672" height="941" fetchPriority="high" alt="Nature’s Superfood — Premium Makhana. Har Bite Mein Sehat, Har Din Mein Behtari. 100% natural, rich in protein, no preservatives. From Bihar’s fields to your home."/><a className="hero-shop-link" href="#products" aria-label="Explore our makhana products"/></section>
   <section className="brand-intro"><div className="brand-leaf"><svg viewBox="0 0 64 48" width="54" height="42" fill="currentColor" aria-hidden="true"><path d="M31 43C7 44 5 25 5 15c15 0 27 8 26 28ZM33 37C30 13 45 3 61 2c0 17-7 34-28 35Z"/><path d="M30 48h4V29h-4z"/></svg></div><h1>MEENA GREEN</h1><h3>A HEALTHIER TOMORROW</h3><p>At Meena Green, we bring you the finest Makhana (Fox Nuts) sourced from the fertile fields of Bihar.<br/>Our mission is to deliver pure, natural and high-quality products to make your snacking healthier and happier.</p><div className="quality-strip">{[['quality-natural.png','Pure & Natural'],['quality-selected.png','Carefully Selected'],['quality-hygiene.png','Hygienically Processed'],['quality-families.png','Loved by Families']].map(([icon,label])=><div key={label}><img src={'/'+icon} alt="" width="52" height="52"/><span><b>{label}</b></span></div>)}</div></section>
   <section className="products section" id="products"><div className="section-heading"><span>🌿</span><div><h2>Our Makhana Range</h2><p>TRADITIONAL GOODNESS, MODERN NUTRITION</p></div><span>🌿</span></div><div className="filters">{filters.map(f=><button className={filter===f?'selected':''} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="cards">{shown.length?shown.map(p=><ProductCard key={p.name} product={p} section="range" liked={liked.includes(p.name)} onLike={()=>toggle(p.name)} onAdd={()=>addToCart(p.name)}/>):<p className="empty">Coming soon — our newest flavours are being prepared.</p>}</div></section>
   <section className="nature"><div className="snack-message"><h2>SNACK HEALTHY<br/>LIVE BETTER</h2><div className="snack-occasions">{[['⌂','At Home'],['▣','At Work'],['✈','While Traveling'],['♡','For Fitness'],['☺','For Kids']].map(([icon,label])=><span key={label}><b>{icon}</b>{label}</span>)}</div></div><blockquote>Small Snacks<br/>Big Happiness</blockquote></section>
   ...
   <section className="bestsellers"><div className="section-heading"><div><h2>Our Bestsellers</h2><p>Loved by Families, Chosen for Good Health</p></div></div><div className="cards">{products.map(p=><ProductCard key={p.name} product={p} section="bestsellers" liked={liked.includes(p.name)} onLike={()=>toggle(p.name)} onAdd={()=>addToCart(p.name)}/>)}</div></section>
